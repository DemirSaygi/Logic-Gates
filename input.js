const writingOffset = {x: 7, y:5};
const distanceToPut = 6;
const moveablePartMetrics = {width: 16, height:55, xOffset:7}
...
class Input {
	constructor(x, y, parentCode, type = 1) {
		...
		this.isPowered =  Array(type).fill(false); //For testing loopingGates
		this.basePower =  Array(type).fill(false); //For using three state buffer
		this.moving = false
		...
		this.colorMode = "red"
		this.tag = this.parentCode == null || this.parentType == "bus" ? "OUT" : "IN"
	...
	calculateHitbox() {
		if(this.parentCode == null) {
			...
			if(this.type == 1) {
				this.hitbox = {
					x: this.x - bigIO_radius - mainPutLineWidth - IO_radius * 2  - hitboxOffset.x,
			...
			if(this.type == 4) {
				this.hitbox = {
					x: this.x - oneSquareSwitchLength - mainPutLineWidth - IO_radius * 2  - hitboxOffset.x,
					y: this.y - this.height/2 - hitboxOffset.y, 
					w: this.width + hitboxOffset.x*2, 
					h: this.height + hitboxOffset.y*2
				}
			...
			if(this.type == 8) {
				this.hitbox = {
					x: this.x - oneSquareSwitchLength*2 - mainPutLineWidth - IO_radius * 2  - hitboxOffset.x,
					y: this.y - this.height/2 - hitboxOffset.y, 
					w: this.width + hitboxOffset.x*2, 
					h: this.height + hitboxOffset.y*2
				}
	...
	getColor(isLit, isBasePower) {
		
		if(!isBasePower && showThreeState) {
			return "rgba(0, 0, 0, 1)"
		}
	...
	show() {
		...
		if (this.parentCode == null) {
			...
			c.beginPath();
			c.fillStyle = this.moveablePartHighlight ? "rgba(150,150,150,1)" : "rgba(80,80,80,1)";
			...
			if(this.type == 1) {	
				...
				c.beginPath();
				c.fillStyle = (this.connectionHighlight || this.hoverHighlight) ? "rgba(255,255,255,1)" : "black";
			...
			if(this.type == 4) {
				...
				for (let i = 0; i < 4; i++) {
					...
					c.lineWidth = 2.5 * camera.scale;
					c.strokeStyle = "black";
					c.fillStyle = this.getColor(this.lit[i], this.basePower[i]);
			...
			if(this.type == 8) {
				...
				c.beginPath();
				c.fillStyle = "black";
				c.rect(screenPoints.x - oneSquareSwitchLength * camera.scale * 2, screenPoints.y - mainPutLineHeight * camera.scale / 2, -(mainPutLineWidth + IO_radius) * camera.scale, mainPutLineHeight * camera.scale);
				c.fill();
				c.closePath();
			...
			if(this.highlight) {
				...
				if(this.type == 1) {
					highlightRect.x = this.x - bigIO_radius - mainPutLineWidth - IO_radius * 2  - highlightOffset

				}
				if(this.type == 4) {				
					highlightRect.x = this.x - oneSquareSwitchLength - mainPutLineWidth - IO_radius * 2  - highlightOffset			
				}
				if(this.type == 8) {
					highlightRect.x = this.x - oneSquareSwitchLength*2 - mainPutLineWidth - IO_radius * 2  - highlightOffset
				}
			...
			if(myP == "Always" || (myP == "On Hover" && this.hoverHighlight) || (myP == "Tab To Toggle" && toggleWithTab)) {
				...
				c.rect(screenPoints.x - (bigIO_radius + mainPutLineWidth + IO_radius * 2 + distanceToPut + extraOffset) * camera.scale - textWidth, screenPoints.y - textHeight / 2 - writingOffset.y, textWidth, textHeight + writingOffset.y * 2);
				c.fill();
				c.fillStyle = "white";
			...
			if(this.type == 1) {
				// Draws the small circle
				c.beginPath();
				c.fillStyle = (this.connectionHighlight || this.hoverHighlight) ? "rgba(255,255,255,1)" : this.getColor(this.lit[0], true);
		...
		if(doesSnap() && isParentAllowed) {
			...
			if(dx != 0 || dy != 0) {
				...
				for(let i of this.inputs) {
					decode(cables, i).move(dx, dy, "Input");
				}		
			...
			this.updateAllPoints()
			for(let i of this.inputs) {
				decode(cables, i).move(addX, addY, "Input");
			}	
	...
	delete() {
		...
		if(findIndex(inputs, this) != null) {
			inputs.splice(findIndex(inputs, this),1);
		}
		rewriteUsedCodes();	
